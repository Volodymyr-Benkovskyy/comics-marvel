...
let i=!1;for(;;)switch(e){case"ascii":case"latin1":case"binary":return r;case"utf8":case"utf-8":return tj(t).length;case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return 2*r;case"hex":return r>>>1;case"base64":return tN(t).length;default:if(i)return n?-1:tj(t).length// assume utf8
...
// ================
const tI=/[^+/0-9A-Za-z-_]/g;function tj(t,e){let r;e=e||1/0;let n=t.length,i=null,o=[];for(let s=0;s<n;++s){// is surrogate component
...
t=tM(t),!r&&e?"["+t+"]":t)}).join(r?".":""):e}const tq=J.toFlatObject(J,{},null,function(t){return/^is[A-Z]/.test(t)});var tz=/**
...
const{Axios:eU,AxiosError:ex,CanceledError:eC,isCancel:eI,CancelToken:ej,VERSION:eN,all:eL,Cancel:e_,isAxiosError:eP,spread:eF,toFormData:ek,AxiosHeaders:eD,HttpStatusCode:eM,formToJSON:e$,getAdapter:eq,mergeConfig:ez}=eT;var eH={};n=d("fSg2G"),i=d("28Nm5").utf8,o=d("c8cSi"),s=d("28Nm5").bin,// Auxiliary functions
 ...
 */function eQ(t,e){return"string"==typeof t?t:t[e%t.length]}const e0=new(0,eG.Spinner)({lines:16,length:18,width:4,radius:20,color:"#600404"}).spin(),e1=()=>{let t=document.getElementById("loader-container");t.innerHTML="",t.appendChild(e0.el),t.style.display="flex"},e2=()=>{let t=document.getElementById("loader-container");t.style.display="none",t.innerHTML=""},e6=document.querySelector(".js-header-search"),e5=document.querySelector(".js-header-form");let e4=null;e4=(()=>{let t={mobile:320,tablet:768};return window.innerWidth<=t.mobile?5:window.innerWidth<=t.tablet?8:16})();const e3=t=>t.map(t=>`<li class="header-search-item">
    ...
    `).join(""),e8=t=>{e6.insertAdjacentHTML("beforeend",e3(t))},e7=()=>{e6.scrollIntoView({behavior:"smooth"})},e9=()=>{e6.innerHTML=""},rt=async t=>{t.preventDefault();let{target:e}=t,r=e.elements.searchQuery.value;if(e5.reset(),""!==r)try{e1();let t=await eJ.getCharacters({nameStartsWith:r,limit:e4,offset:0});if(0===t.results.length){e2(),e7(),e6.innerHTML='<div class="nothing-seach"></div>';return}e2(),e9(),e8(t.results),e7()}catch(t){location.replace("../error.html"),e2()}};e5.addEventListener("submit",rt);const re=document.querySelector(".header");window.addEventListener("scroll",()=>{0!==scrollY?re.classList.add("js-header-bg"):re.classList.remove("js-header-bg")}),new Swiper(".section-hero-swiper",{direction:"vertical",loop:!0,pagination:{el:".swiper-pagination",clickable:!0},spaceBetween:50,slidesPerView:1,initialSlide:0});const rr=document.querySelector(".swiper-pagination"),rn=document.querySelector(".js-btn-color");rr.addEventListener("click",t=>{let{target:e}=t,r=e.getAttribute("aria-label"),n="";"Go to slide 1"===r?(n="#600404",rn.style.background=n):"Go to slide 2"===r?(n="#34387f",rn.style.background=n):"Go to slide 3"===r&&(n="#5b7f3c",rn.style.background=n)});const ri=document.querySelector(".js-random-img"),ro=document.querySelector(".js-random-descr"),rs=t=>{let e=t.map(({id:t,thumbnail:e,name:r})=>`  <li class= "js-slide js-random-characters-img" data-id="${t}">
      ...
      <img class="js-random-characters-img" data-id="${t}"
        ...
        alt='${r}'
      /></picture>
    </li>`).join("");return e},ra=t=>{ri.insertAdjacentHTML("beforeend",rs(t))},ru=t=>{let e=t.map(({name:t,description:e,id:r})=>`
    ...
    </li>`).join("");return e},rf=t=>{ro.insertAdjacentHTML("beforeend",ru(t))},rl=async()=>{try{let t=await eJ.getCharacters("/characters",{params:{limit:5,offset:0}}),{results:e}=t;e=e.filter(({description:t,thumbnail:{path:e}})=>t.length&&!e.includes("not_available"));let r=[];for(let t=0;t<5;t+=1){let t=Math.floor(Math.random()*e.length),n=e[t];r.includes(n)||r.push(n)}console.log(r),ra(r),rf(r)}catch(t){return console.log(t),{error:t.message}}};rl();//# sourceMappingURL=index.f884f1a5.js.map

//# sourceMappingURL=index.f884f1a5.js.map
