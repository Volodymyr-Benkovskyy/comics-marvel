...
// ================
let tj=/[^+/0-9A-Za-z-_]/g;function tI(t,e){let r;e=e||1/0;let n=t.length,i=null,o=[];for(let s=0;s<n;++s){// is surrogate component
 ...
 *
 * @returns {boolean}
 */function t$(t){return V.isPlainObject(t)||V.isArray(t)}/**
 * It removes the brackets from the end of a string
 *
 * @param {string} key - The key of the parameter.
 *
 * @returns {string} the key without the brackets.
 */function tk(t){return V.endsWith(t,"[]")?t.slice(0,-2):t}/**
...
t=tk(t),!r&&e?"["+t+"]":t)}).join(r?".":""):e}let tq=V.toFlatObject(V,{},null,function(t){return/^is[A-Z]/.test(t)});var tz=/**
...
t=JSON.stringify(t);else{var l;if(V.isArray(t)&&(l=t,V.isArray(l)&&!l.some(t$))||(V.isFileList(t)||V.endsWith(r,"[]"))&&(a=V.toArray(t)))return(// eslint-disable-next-line no-param-reassign
r=tk(r),a.forEach(function(t,n){V.isUndefined(t)||null===t||e.append(!0===s?tM([r],n,o):null===s?r:r+"[]",u(t))}),!1)}}return!!t$(t)||(e.append(tM(i,r,o),u(t)),!1)}let c=[],h=Object.assign(tq,{defaultVisitor:f,convertValue:u,isVisitable:t$});if(!V.isObject(t))throw TypeError("data must be an object");return!function t(r,n){if(!V.isUndefined(r)){if(-1!==c.indexOf(r))throw Error("Circular reference detected in "+n.join("."));c.push(r),V.forEach(r,function(r,o){let s=!(V.isUndefined(r)||null===r)&&i.call(e,r,V.isString(o)?o.trim():o,n,h);!0===s&&t(r,n?n.concat(o):[o])}),c.pop()}}(t),e};/**
...
let{Axios:ex,AxiosError:eU,CanceledError:eC,isCancel:ej,CancelToken:eI,VERSION:eL,all:eN,Cancel:e_,isAxiosError:eP,spread:eF,toFormData:eD,AxiosHeaders:e$,HttpStatusCode:ek,formToJSON:eM,getAdapter:eq,mergeConfig:ez}=eT;var eH={};n=d("19JXX"),i=d("h0JH0").utf8,o=d("gixTh"),s=d("h0JH0").bin,// Auxiliary functions
...
let rr=document.querySelector(".js-random-img"),rn=document.querySelector(".js-random-descr"),ri=t=>{let e=t.map(({id:t,thumbnail:e,name:r})=>`  <li class= "js-slide js-random-characters-img" data-id="${t}">
      <picture>
    ...
    </li>`).join("");return e},ra=t=>{rn.insertAdjacentHTML("beforeend",rs(t))},rl=async()=>{try{let t=await eV.getCharacters("/characters"),{results:e}=t;e=e.filter(({name:t,thumbnail:{path:e}})=>t&&!e.includes("not_available"));let r=[],n=[...e];for(let t=0;t<5&&n.length>0;t++){let t=Math.floor(Math.random()*n.length),e=n[t];r.push(e),n.splice(t,1)}console.log(r),ro(r),ra(r),ru=document.querySelectorAll(".js-slide"),rf=document.querySelectorAll(".js-random-characters-description"),setInterval(rp,3500)}catch(t){return console.log(t),location.replace("../error.html"),{error:t.message}}};rl();let ru=[],rf=null,rc=0,rh=0,rp=()=>{for(let t=0;t<ru.length;t+=1)ru[t].classList.remove("js-show"),ru[t].classList.add("js-slide");for(let t=0;t<rf.length;t+=1)rf[t].classList.remove("js-active"),rf[t].classList.add("js-random-characters-description");rh=(rh+1)%ru.length,rc=(rc+1)%rf.length,ru[rh].classList.add("js-show"),ru[rh].classList.remove("js-slide"),rf[rc].classList.add("js-active"),rf[rc].classList.remove("js-random-characters-description")},rd=document.querySelector(".js-last-comics-list"),rg=t=>{let e=t.map(({id:t,thumbnail:e,title:r,creators:n})=>`  <li class= "last-comics-item swiper-slide" data-id="${t}" width="auto">
     <picture>
      <source media="(min-width: 1440px)"
                  srcset="${e.path}.${e.extension}" />
      <source media="(min-width: 768px)"
    ...
    </li>`).join("");return e},ry=t=>{rd.insertAdjacentHTML("beforeend",rg(t))},rm=async()=>{try{let t=await eV.getComics({limit:3,dateDescriptor:"lastWeek"});ry(t.results)}catch(t){console.log(t)}};rm(),new Swiper(".section-comics-swiper",{slidesPerView:1,spaceBetween:15,scrollbar:{el:".swiper-scrollbar",draggable:!0},breakpoints:{375:{slidesPerView:1},768:{slidesPerView:1.5},1440:{slidesPerView:3,init:!1}}})}();//# sourceMappingURL=index.44db52ce.js.map

//# sourceMappingURL=index.44db52ce.js.map
