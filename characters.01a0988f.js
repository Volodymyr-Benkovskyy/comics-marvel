 ...
 *     dh: 'dh'
 * });
 */function(t,e){var n=document.createElement("img"),r="";return i(e,function(t,e){r+="&"+e+"="+t}),r=r.substring(1),n.src=t+"?"+r,n.style.display="none",document.body.appendChild(n),document.body.removeChild(n),n};/***/}]);let M=document.querySelector(".js-characters-sort-container"),L=document.querySelector(".js-characters-sort-form");document.getElementById("characters-list");let w=document.querySelector(".js-chararcters-comics-input");document.querySelector(".js-characters-select-order");let I=document.querySelector(".js-chararcters-name-input");document.querySelector(".js-header-chararcters-name"),document.querySelector(".js-header-chararcters-input");let B=[],T=null,N=null;T=(0,a.getItemsPerPage)();let O=t=>t.map(t=>`<li class="characters-sort-item">
    ...
    `).join(""),j=()=>{M.innerHTML=""},S=t=>{M.insertAdjacentHTML("beforeend",O(t))},A=document.querySelector(".tui-pagination"),H={totalItems:0,itemsPerPage:T,visiblePages:3},k=new/*@__PURE__*/(t(E))(A,H),F=async()=>{try{s.showLoader;let t=await (0,o.api).getCharacters({offset:0,limit:T});if(console.log("fetchAndRenderCharacterList =>",t),(0,s.hideLoader)(),S(t.results),A.classList.remove("is-hidden"),T>t.total)return;k.reset(t.total),k.on("beforeMove",async t=>{let e=t.page,n=T*(e-1);try{(0,s.showLoader)();let t=await (0,o.api).getCharacters({offset:n,limit:T,nameStartsWith:N});(0,s.hideLoader)(),j(),S(t.results)}catch(t){console.log("Error"),(0,s.hideLoader)()}})}catch(t){console.log(t)}};F();/* ////////////////////    / RANDOM  /  ////////////////////////////// *//* ////////////////     SEARCH-INPUT-COMICS   //////////////////////////////////// */let q=async t=>{(0,s.showLoader)(),t.preventDefault();let{target:e}=t;queryComics=e.elements.searchComics.value,B=[],N=null,queryOrder=null,dateQuery=null;try{let t=await (0,o.api).getComics({title:queryComics,offset:0,limit:100});if(0===t.total){(0,s.hideLoader)(),w.value="",A.classList.add("is-hidden");return}let e=t.results.slice(0,10);e.map(t=>{t.characters.available&&B.push(t.id)}),console.log("onSearchComics => getComics=>",t),arrayIdComics=B.join(", ");try{let t=await (0,o.api).getCharacters({nameStartsWith:N,comics:arrayIdComics,limit:T});if(0===t.length){A.classList.add("is-hidden");return}(0,s.hideLoader)(),j(),k.reset(t.total),S(t.results),A.classList.remove("is-hidden"),w.value="",console.log("onSearchComics => getCharacters",t)}catch(t){console.log("Error!",t)}}catch(t){(0,s.hideLoader)(),console.log("Error!",t)}};L.addEventListener("submit",q);/* ////////////////    / SEARCH-INPUT-COMICS  /   //////////////////////////////////// *//* ///////////////////   SEARCH-NAME  ///////////////////////////////////// */let $=async t=>{A.classList.add("is-hidden"),N=t.target.value;try{(0,s.showLoader)();let t=await (0,o.api).getCharacters({offset:0,limit:T,nameStartsWith:N});if(0===t.total){(0,s.hideLoader)(),A.classList.add("is-hidden");return}console.log("onSearchName =>response",t),(0,s.hideLoader)(),k.reset(t.total),j(),S(t.results),A.classList.remove("is-hidden"),I.value=""}catch(t){console.log("Error!",t)}};I.addEventListener("input",/*@__PURE__*/t(c)($,2e3))}();//# sourceMappingURL=characters.01a0988f.js.map

//# sourceMappingURL=characters.01a0988f.js.map
