...
// ================
let tC=/[^+/0-9A-Za-z-_]/g;function tI(t,e){let r;e=e||1/0;let n=t.length,i=null,o=[];for(let s=0;s<n;++s){// is surrogate component
...
let{Axios:ex,AxiosError:eU,CanceledError:ej,isCancel:eC,CancelToken:eI,VERSION:eL,all:eN,Cancel:e_,isAxiosError:eP,spread:eF,toFormData:eD,AxiosHeaders:ek,HttpStatusCode:e$,formToJSON:eM,getAdapter:eq,mergeConfig:ez}=eT;var eH={};n=d("19JXX"),i=d("h0JH0").utf8,o=d("gixTh"),s=d("h0JH0").bin,// Auxiliary functions
...
rr&&rr.destroy(),rr=new Swiper(".section-hero-swiper",{direction:n,loop:!1,pagination:{el:".swiper-pagination",clickable:!0},spaceBetween:50,slidesPerView:1,initialSlide:0,allowTouchMove:!1})}rn(),window.addEventListener("resize",rn);let ri=document.querySelector(".swiper-pagination"),ro=document.querySelector(".js-btn-color");ri.addEventListener("click",t=>{let{target:e}=t,r=e.getAttribute("aria-label"),n="";"Go to slide 1"===r?(n="#600404",ro.style.background=n):"Go to slide 2"===r?(n="#34387f",ro.style.background=n):"Go to slide 3"===r&&(n="#5b7f3c",ro.style.background=n)});let rs=document.querySelector(".js-random-img"),ra=document.querySelector(".js-random-descr"),rl=t=>{let e=t.map(({id:t,thumbnail:e,name:r})=>`  <li class= "js-slide js-random-characters-img" data-id="${t}">
      <picture>
...
r.forEach(t=>{t.thumbnail.path.startsWith("https://")||(t.thumbnail.path=t.thumbnail.path.replace("http://","https://"))}),console.log(r),ru(r),rc(r),rp=document.querySelectorAll(".js-slide"),rd=document.querySelectorAll(".js-random-characters-description"),setInterval(rm,3500)}catch(t){return console.log(t),location.replace("../error.html"),{error:t.message}}};rh();let rp=[],rd=null,rg=0,ry=0,rm=()=>{for(let t=0;t<rp.length;t+=1)rp[t].classList.remove("js-show"),rp[t].classList.add("js-slide");for(let t=0;t<rd.length;t+=1)rd[t].classList.remove("js-active"),rd[t].classList.add("js-random-characters-description");ry=(ry+1)%rp.length,rg=(rg+1)%rd.length,rp[ry].classList.add("js-show"),rp[ry].classList.remove("js-slide"),rd[rg].classList.add("js-active"),rd[rg].classList.remove("js-random-characters-description")},rb=document.querySelector(".js-last-comics-list"),rw=t=>{let e=t.map(({id:t,thumbnail:e,title:r,creators:n})=>`  <li class= "last-comics-item swiper-slide" data-id="${t}" width="auto">
    ...
    </li>`).join("");return e},rE=t=>{rb.insertAdjacentHTML("beforeend",rw(t))},rv=async()=>{try{let t=await eV.getComics({limit:3,dateDescriptor:"lastWeek"});// Перевірка HTTPS для зображень у відповіді
t.results.forEach(t=>{t.thumbnail.path.startsWith("https://")||(t.thumbnail.path=t.thumbnail.path.replace("http://","https://"))}),rE(t.results)}catch(t){console.log(t)}};rv(),new Swiper(".section-comics-swiper",{slidesPerView:1,spaceBetween:15,scrollbar:{el:".swiper-scrollbar",draggable:!0},breakpoints:{375:{slidesPerView:1},768:{slidesPerView:1.5},1440:{slidesPerView:3,init:!1}}})}();//# sourceMappingURL=index.cc6f58ca.js.map

//# sourceMappingURL=index.cc6f58ca.js.map
