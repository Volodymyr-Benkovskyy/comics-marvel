...
delete this.name}get code(){return t}set code(t){Object.defineProperty(this,"code",{configurable:!0,enumerable:!0,value:t,writable:!0})}toString(){return`${this.name} [${t}]: ${this.message}`}}}function tT(t){let e="",r=t.length,n="-"===t[0]?1:0;for(;r>=n+4;r-=3)e=`_${t.slice(r-3,r)}${e}`;return`${t.slice(0,r)}${e}`}function tx(t,e,r,n,i,o){if(t>r||t<e){let n;let i="bigint"==typeof e?"n":"";throw n=o>3?0===e||e===BigInt(0)?`>= 0${i} and < 2${i} ** ${(o+1)*8}${i}`:`>= -(2${i} ** ${(o+1)*8-1}${i}) and < 2 ** ${(o+1)*8-1}${i}`:`>= ${e}${i} and <= ${r}${i}`,new tS.ERR_OUT_OF_RANGE("value",n,t)}tU(i,"offset"),(void 0===n[i]||void 0===n[i+o])&&tj(i,n.length-(o+1))}function tU(t,e){if("number"!=typeof t)throw new tS.ERR_INVALID_ARG_TYPE(e,"number",t)}function tj(t,e,r){if(Math.floor(t)!==t)throw tU(t,r),new tS.ERR_OUT_OF_RANGE(r||"offset","an integer",t);if(e<0)throw new tS.ERR_BUFFER_OUT_OF_BOUNDS;throw new tS.ERR_OUT_OF_RANGE(r||"offset",`>= ${r?1:0} and <= ${e}`,t)}tR("ERR_BUFFER_OUT_OF_BOUNDS",function(t){return t?`${t} is outside of buffer bounds`:"Attempt to access memory outside buffer bounds"},RangeError),tR("ERR_INVALID_ARG_TYPE",function(t,e){return`The "${t}" argument must be of type number. Received type ${typeof e}`},TypeError),tR("ERR_OUT_OF_RANGE",function(t,e,r){let n=`The value of "${t}" is out of range.`,i=r;return Number.isInteger(r)&&Math.abs(r)>4294967296?i=tT(String(r)):"bigint"==typeof r&&(i=String(r),(r>BigInt(2)**BigInt(32)||r<-(BigInt(2)**BigInt(32)))&&(i=tT(i)),i+="n"),n+=` It must be ${e}. Received ${i}`},RangeError);// HELPER FUNCTIONS
// ================
const tC=/[^+/0-9A-Za-z-_]/g;function tI(t,e){let r;e=e||1/0;let n=t.length,i=null,o=[];for(let s=0;s<n;++s){// is surrogate component
...
const{Axios:ex,AxiosError:eU,CanceledError:ej,isCancel:eC,CancelToken:eI,VERSION:eL,all:eN,Cancel:e_,isAxiosError:eP,spread:eF,toFormData:eD,AxiosHeaders:ek,HttpStatusCode:e$,formToJSON:eM,getAdapter:eq,mergeConfig:ez}=eT;var eH={};n=d("fSg2G"),i=d("28Nm5").utf8,o=d("c8cSi"),s=d("28Nm5").bin,// Auxiliary functions
...
window.addEventListener('resize', initSwiper); */let rr=null;// Змінна для зберігання екземпляру Swiper
// Функція для ініціалізації Swiper
...
rr&&rr.destroy(),rr=new Swiper(".section-hero-swiper",{direction:n,loop:!1,pagination:{el:".swiper-pagination",clickable:!0},spaceBetween:50,slidesPerView:1,initialSlide:0,allowTouchMove:!1})}rn(),window.addEventListener("resize",rn);const ri=document.querySelector(".swiper-pagination"),ro=document.querySelector(".js-btn-color");ri.addEventListener("click",t=>{let{target:e}=t,r=e.getAttribute("aria-label"),n="";"Go to slide 1"===r?(n="#600404",ro.style.background=n):"Go to slide 2"===r?(n="#34387f",ro.style.background=n):"Go to slide 3"===r&&(n="#5b7f3c",ro.style.background=n)});const rs=document.querySelector(".js-random-img"),ra=document.querySelector(".js-random-descr"),rl=t=>{let e=t.map(({id:t,thumbnail:e,name:r})=>`  <li class= "js-slide js-random-characters-img" data-id="${t}">
      <picture>
      ...
      <img class="js-random-characters-img" data-id="${t}"
        ...
        alt='${r}'
      /></picture>
    </li>`).join("");return e},ru=t=>{rs.insertAdjacentHTML("beforeend",rl(t))},rf=t=>{let e=t.map(({name:t,description:e,id:r})=>`
    ...
    </li>`).join("");return e},rc=t=>{ra.insertAdjacentHTML("beforeend",rf(t))},rh=async()=>{try{let t=await eJ.getCharacters("/characters"),{results:e}=t;e=e.filter(({name:t,thumbnail:{path:e}})=>t&&!e.includes("not_available"));let r=[],n=[...e];for(let t=0;t<5&&n.length>0;t++){let t=Math.floor(Math.random()*n.length),e=n[t];r.push(e),n.splice(t,1)}// Ваша логіка для перевірки HTTPS і зміни URL тут
r.forEach(t=>{t.thumbnail.path.startsWith("https://")||(t.thumbnail.path=t.thumbnail.path.replace("http://","https://"))}),console.log(r),ru(r),rc(r),rp=document.querySelectorAll(".js-slide"),rd=document.querySelectorAll(".js-random-characters-description"),setInterval(rm,3500)}catch(t){return console.log(t),location.replace("../error.html"),{error:t.message}}};rh();let rp=[],rd=null,rg=0,ry=0;const rm=()=>{for(let t=0;t<rp.length;t+=1)rp[t].classList.remove("js-show"),rp[t].classList.add("js-slide");for(let t=0;t<rd.length;t+=1)rd[t].classList.remove("js-active"),rd[t].classList.add("js-random-characters-description");ry=(ry+1)%rp.length,rg=(rg+1)%rd.length,rp[ry].classList.add("js-show"),rp[ry].classList.remove("js-slide"),rd[rg].classList.add("js-active"),rd[rg].classList.remove("js-random-characters-description")},rb=document.querySelector(".js-last-comics-list"),rw=t=>{let e=t.map(({id:t,thumbnail:e,title:r,creators:n})=>`  <li class= "last-comics-item swiper-slide" data-id="${t}" width="auto">
     <picture>
      <source media="(min-width: 1440px)"
                  srcset="${e.path}.${e.extension}" />
      <source media="(min-width: 768px)"
...
t.results.forEach(t=>{t.thumbnail.path.startsWith("https://")||(t.thumbnail.path=t.thumbnail.path.replace("http://","https://"))}),rE(t.results)}catch(t){console.log(t)}};rv(),new Swiper(".section-comics-swiper",{slidesPerView:1,spaceBetween:15,scrollbar:{el:".swiper-scrollbar",draggable:!0},breakpoints:{375:{slidesPerView:1},768:{slidesPerView:1.5},1440:{slidesPerView:3,init:!1}}});//# sourceMappingURL=index.51dc0715.js.map

//# sourceMappingURL=index.51dc0715.js.map
